...
export async function POST(req: NextRequest) {
  try {
    ...
    const { token, servico, motivo, nps, oque, mensagem } = body;
    ...
    const { error: insertErr } = await supabase.from('quiz_encerramento_responses').insert({
      token,
      timestamp: new Date().toISOString(),
      nome: tokenData.cliente_nome,
      clinica: tokenData.clinica,
      servico_utilizado: servico,
      motivo_encerramento: motivo,
      nps,
      o_que_poderia_mudar: oque ?? '',
    ...
    console.error('[submit-encerramento]', err);
