...
export default function QuizTreinamentoClient({ token, nome, clinica, ferramenta }: Props) {
  const [step, setStep] = useState(0);
  const [respondente, setRespondente] = useState('');
  const [notaTreinamento, setNotaTreinamento] = useState<number | null>(null);
  ...
  async function handleSubmit() {
    ...
    setError('');
    try {
      const res = await fetch('/api/submit-nps-treinamento', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token, respondente, notaTreinamento, notaClareza, segurancaUso: seguranca, observacoes }),
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          {ferramenta && (
            <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-xs bg-[rgba(155,89,182,0.15)] text-[#C39BD3] border border-[rgba(155,89,182,0.25)]">
      ...
      <div className="w-full h-1 bg-[#1A1A2E] rounded-full overflow-hidden">
        <div
          ...
          style={{ width: `${((step + 1) / TOTAL) * 100}%`, background: 'linear-gradient(90deg, #9B59B6, #C39BD3)' }}
        />
      </div>
      ...
      {step === 1 && (
        <ScaleQuestion
          label="Como você avalia o treinamento de hoje?"
          value={notaTreinamento}
          onChange={setNotaTreinamento}
        />
      )}

      {step === 2 && (
        <ScaleQuestion
          label="O conteúdo foi claro e fácil de aplicar?"
          value={notaClareza}
      ...
      {step === 4 && (
        <div className="space-y-4">
          ...
          <textarea
            ...
            rows={4}
            className="w-full bg-[#12122A] border border-[rgba(155,89,182,0.3)] rounded-xl px-4 py-3 text-white placeholder-[#555570] focus:outline-none focus:border-[#9B59B6] transition-all font-sora text-sm resize-none"
          />
        </div>
      )}
