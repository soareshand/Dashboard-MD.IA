...
const OBJETIVOS_OPTIONS = [
  'Organizar o relacionamento com pacientes',
...
const CRM_TOOL = { id: 'notaCRM', label: 'Ferramenta CRM', emoji: '📇' };
const AGENTE_TOOL = { id: 'notaAgenteIA', label: 'Agente de IA (Secretária)', emoji: '🤖' };
...
interface FormData {
  ...
  maiorDesafio: string;
  notaCRM: number | null;
  crmMelhorouRelacionamento: string;
  notaAgenteIA: number | null;
  agenteIAMelhorouAtendimento: string;
  agenteIAUsoPrincipal: string;
...
export default function QuizClientIG({ token, initial }: { token: string; initial: InitialData }) {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    maiorDesafio: '',
    notaCRM: null,
    crmMelhorouRelacionamento: '',
    notaAgenteIA: null,
    agenteIAMelhorouAtendimento: '',
    agenteIAUsoPrincipal: '',
  ...
  function canAdvance(): boolean {
    ...
    if (step === 3) return form.notaCRM !== null && !!form.crmMelhorouRelacionamento;
    if (step === 4) return form.notaAgenteIA !== null && !!form.agenteIAMelhorouAtendimento && !!form.agenteIAUsoPrincipal.trim();
    if (step === 5) return !!(form.crescimentoPacientes && form.reducaoTempoOperacional && form.investimentoRetorno);
    if (step === 6) {
  ...
  async function handleSubmit() {
    ...
    setError('');
    try {
      const res = await fetch('/api/submit-quiz-ig', {
        method: 'POST',
...
function Step2({ form, update }: { form: FormData; update: (f: keyof FormData, v: string | number | null) => void }) {
  return (
    <>
      ...
      <QuestionBlock>
        <Label>
          Qual foi o seu principal objetivo ao contratar o Infinite Gear?{' '}
          <span className="text-[#555570] text-xs">(pode selecionar mais de um)</span>
        </Label>
        <OptionSelector
      ...
      <QuestionBlock>
        <Label>
          Qual é o seu maior desafio hoje na clínica?{' '}
          <span className="text-[#555570] text-xs">(pode selecionar mais de um)</span>
        </Label>
        <OptionSelector
...
function Step3({ form, update }: { form: FormData; update: (f: keyof FormData, v: string | number | null) => void }) {
  return (
    <>
      <StepTitle>Experiência com o CRM</StepTitle>
      <p className="text-xs text-[#A0A0B0] mb-6">Avalie de 0 (Muito Insatisfeito) a 5 (Muito Satisfeito)</p>
      <QuestionBlock>
        <ScaleInput item={CRM_TOOL} value={form.notaCRM} onChange={v => update('notaCRM', v)} />
      </QuestionBlock>
      <QuestionBlock>
        <Label>O CRM melhorou a organização do relacionamento com seus pacientes?</Label>
        <ChoiceButtons
          options={['Sim', 'Parcialmente', 'Não']}
          value={form.crmMelhorouRelacionamento}
          onChange={v => update('crmMelhorouRelacionamento', v)}
        />
      </QuestionBlock>
    </>
  );
...
function Step4({ form, update }: { form: FormData; update: (f: keyof FormData, v: string | number | null) => void }) {
  return (
    <>
      <StepTitle>Experiência com o Agente de IA</StepTitle>
      <p className="text-xs text-[#A0A0B0] mb-6">Avalie de 0 (Muito Insatisfeito) a 5 (Muito Satisfeito)</p>
      <QuestionBlock>
        <ScaleInput item={AGENTE_TOOL} value={form.notaAgenteIA} onChange={v => update('notaAgenteIA', v)} />
      </QuestionBlock>
      <QuestionBlock>
        <Label>O Agente de IA melhorou o atendimento e comunicação com pacientes?</Label>
        <ChoiceButtons
          options={['Sim', 'Parcialmente', 'Não']}
          value={form.agenteIAMelhorouAtendimento}
          onChange={v => update('agenteIAMelhorouAtendimento', v)}
        />
      </QuestionBlock>
      <QuestionBlock>
        <Label>
          Para que o Agente de IA é mais utilizado na sua clínica?{' '}
          <span className="text-[#555570] text-xs">(pode selecionar mais de um)</span>
        </Label>
        <OptionSelector
          options={AGENTE_USO_OPTIONS}
          value={form.agenteIAUsoPrincipal}
          onChange={v => update('agenteIAUsoPrincipal', v)}
...
function Step6({ form, update }: { form: FormData; update: (f: keyof FormData, v: string | number | null) => void }) {
  return (
    <>
      ...
      <QuestionBlock>
        <Label>Pretende renovar o contrato com o Infinite Gear?</Label>
        <ChoiceButtons
          options={['Sim', 'Não']}
      ...
      <QuestionBlock>
        <Label>Você indicaria o Infinite Gear para outro médico?</Label>
        <ChoiceButtons
          options={['Sim', 'Não']}
...
function Step7({ form, update }: { form: FormData; update: (f: keyof FormData, v: string | number | null) => void }) {
  return (
    <>
    ...
    <div className="flex flex-col items-center justify-center py-12 animate-fade-up text-center">
      <div className="w-24 h-24 mb-8 relative">
        <svg viewBox="0 0 100 100" className="w-full h-full">
          <circle cx="50" cy="50" r="46" fill="none" stroke="url(#successGradIG)" strokeWidth="3" />
          <defs>
            <linearGradient id="successGradIG" x1="0%" y1="0%" x2="100%" y2="100%">
